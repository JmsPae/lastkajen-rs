...
impl fmt::Display for LastkajenError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::LastkajenError(err) => write!(f, "Lastkajen Error: {}", err)
...
impl Lastkajen {
    ...
    pub async fn new(user_name: String, password: String) -> Result<Self> {
        ...
        Ok(Self {
            #[cfg(feature = "time")]
            expiry_date_time: time::OffsetDateTime::now_utc().saturating_add(time::Duration::seconds(token.expires_in as i64)),
    ...
    pub async fn retrieve_token(user_name: String, password: String) -> Result<types::Token> {
        ...
        let res = client.post("https://lastkajen.trafikverket.se/api/Identity/Login").form(&params).send().await?;
    ...
    pub async fn get_published_packages(&self) -> Result<Vec<types::DataPackageFolder>> {
        let res = self.client.get("https://lastkajen.trafikverket.se/api/DataPackage/GetPublishedDataPackages")
                             .bearer_auth(&self.token.access_token).send().await?;
    ...
    pub async fn get_package_files(&self, package: &types::DataPackageFolder) -> Result<Vec<types::DataPackageFile>> {
        Ok(self.get_package_files_from_id(&package.id).await?)
    }

    /// Get information and download links for a data package.
    pub async fn get_package_files_from_id(&self, id: &usize) -> Result<Vec<types::DataPackageFile>> {
        let res = self.client.get(format!("https://lastkajen.trafikverket.se/api/DataPackage/GetDataPackageFiles/{}", id))
                             .bearer_auth(&self.token.access_token).send().await?;
    ...
    pub async fn get_user_files(&self) -> Result<Vec<types::UserFile>> {
        let res = self.client.get("https://lastkajen.trafikverket.se/api/file/GetUserFiles")
                             .bearer_auth(&self.token.access_token).send().await?;
    ...
    pub async fn get_download_token(&self, category: types::DownloadCategory<'_>) -> Result<types::DownloadToken> {
        ...
        let res = Lastkajen::check_status( self.client.get(url).bearer_auth(&self.token.access_token).send().await? ).await?;

        return match category {
            types::DownloadCategory::User { .. } => Ok(types::DownloadToken::User(res.json().await?)),
            types::DownloadCategory::Published { .. } => Ok(types::DownloadToken::Published(res.json().await?))
        };
    }


    /// Download data, expending a download token.
    pub async fn download_with_token(&self, download_token: types::DownloadToken, writable: &mut dyn Write) -> Result<()>  {
        let url: String = match download_token { // No, they aren't interchangable for some reason.
            types::DownloadToken::User(dltoken) => format!("https://lastkajen.trafikverket.se/api/file/GetFileStream?token={}", dltoken),
            types::DownloadToken::Published(dltoken) => format!("https://lastkajen.trafikverket.se/api/file/GetDataPackageFile?token={}", dltoken),
        };
    ...
    pub async fn download_file(&self, category: types::DownloadCategory<'_>, writable: &mut dyn Write) -> Result<()>  {
        let download_token = self.get_download_token(category).await?;
        Ok(self.download_with_token(download_token, writable).await?)
    }
}
