...
async fn main() -> Result<(), Box<dyn Error>> {
    ...
    let gavle_result: Vec<DataPackageFolder> = packages.into_iter().filter(|package| package.source_folder == "Datapaket\\Länsfiler NVDB-data\\Gävleborgs län").collect();
    let gavle_result = gavle_result.first().expect("Couldn't find the right package!");
    ...
    let gpkg: Vec<lastkajen::types::DataPackageFile> = files.into_iter().filter(|file| file.name == "Gävleborgs_län_GeoPackage.zip").collect();
